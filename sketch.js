window.addEventListener('load', () => {
    ...
    function playInstrument(instrument, prediction, yPercent) {
        ...
        // 1. Extract ML Parameters (with defaults if offline)
    ...
    function analyzeAndPlay(stroke) {
        ...
        statusMsg.innerText = "Analyzing shape...";
        statusMsg.style.color = stroke.color;

        // 2. CALL BACKEND API
        fetch('http://127.0.0.1:5000/api/predict_sound', { 
            method: 'POST',
        ...
        .then(prediction => {
            // 3. SUCCESS: PLAY SOUND WITH ML PARAMS
            statusMsg.innerText = `AI: ${shape.toUpperCase()} (Filter: ${prediction.filter_cutoff.toFixed(0)}Hz)`;
        ...
        .catch(error => {
            console.error(error);
            statusMsg.innerText = "AI Offline: Using basic mode";
            // Fallback: Play sound with default empty params
            playInstrument(instrument, {}, yPercent);
    ...
    stopBeatsBtn.addEventListener('click', () => { activeBeats = { kick: false, snare: false, hihat: false }; });
